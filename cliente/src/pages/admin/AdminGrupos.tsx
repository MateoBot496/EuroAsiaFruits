...
export default function AdminGrupos() {
      ...
      if (modo === "crear") {
        try {
          const res = await fetch(
            "http://localhost:3000/api/admin/catalogos?tipo=grupos",
            {
              method: "POST",
              headers: {
        ...
        if (!categoriaActual) return;
            ...
            alert(`Grupo renombrado a "${data.valor}"`);
            ...
            console.error("Error editando grupo:", error.message);
            alert(error.message);
          }
      }
  ...
  const eliminarCategoria = async (categoria: any) => {
    ...
    try {
    const res = await fetch(
      `http://localhost:3000/api/admin/catalogos/active?tipo=grupos`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
    ...
    alert(`Grupo ${categoria.is_active ? "desactivado" : "activado"}: ${categoria.nombre}`);
  ...
  return (
    <div className="flex">
      ...
      <div className="p-4 w-full">
        ...
        <p>Aquí puedes gestionar los grupos de productos.</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
          {catalogos?.grupos.map((categoria) => (
            <div key={categoria.id_grupo} className="border p-4 rounded">
